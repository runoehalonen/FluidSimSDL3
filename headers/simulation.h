...
static inline void applyGravityAndVelocity(Simulation *simulation, float dT);
...
static inline void simulationInit(Simulation *simulation, int boxWidth, int boxHeight, int particleAmount)
{
    ...
    simulation->maxForceRadius = 30.0f;
    // used to prevent simulation going crazy
    simulation->maxVelocity = 1000.0f;
    ...
    const int layerConstant = 10;
    const int layerHeight = 30;
    ...
    {   
        ...
        for (int x = 0; x < createAmount; x++)
        {
            ...
            simulation->particlePool[newIndex].viscosity = 0.4f;
...
static inline void updateSimulation(Simulation *simulation, float deltaTime)
{
    // one update cycle
    ...
    applyGravityAndVelocity(simulation, dT);
...
static inline void applyGravityAndVelocity(Simulation *simulation, float dT)
{
    ...
    for (int i = 0; i < simulation->particlePoolSize; i++)
    {
        ...
        simulation->particlePool[i].velocityY += simulation->gravity * dT;
                ...
                while (currentParticleIndex != -1)
                {
                    if (currentParticleIndex != i)
                    {
                        ...
                        if (dist > 0.0f && dist < r)
                        {
                            ...
                            //float slope = -6.0f * dist * (r*r - dist*dist) * (r*r - dist*dist) / (r*r*r*r*r*r);
                            float slope = -3.0f * (r - dist) * (r - dist) / (r * r * r * r * r);

                            // equalized for both particles.
                            float sharedPressure = (pressureFromDensity(particle->density) + pressureFromDensity(neighborParticle->density)) * 0.5f;
...
static inline float pressureFromDensity(float density)
{
    // used to settle particles when at rest but basically unnecessary
